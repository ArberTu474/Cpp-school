...
int main()
{
  ...
  std::cout << "Enter the starting temperature: ";
  std::cin >> starting_temp;

  std::cout << "Enter the ending temperature: ";
  std::cin >> ending_temp;

  for (int i = starting_temp; i <= ending_temp; i++)
  {
    std::printf("At %d degrees Celsius the velocity of sound is %.1lf m/s\n", i, 331.3 + 0.61 * i);
  }
